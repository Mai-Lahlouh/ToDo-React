...
export default function ToDo({todo}){
    ...
    return(
        <div className='task'onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
            <IconButton onClick={handleCheckClick}>
                {todo.isCompleted ? <CheckCircleIcon className='checkbtn'/> : <CircleOutlinedIcon className='checkbtn'/> }             
            </IconButton>           
            <span className={changeStyle}>{todo.title}</span>     
            <IconButton onClick={handleDeleteTodo}>
                <HighlightOffIcon className={iconBtn ? 'deleteIcon' : 'hide'}></HighlightOffIcon>
            </IconButton>
        </div>
    );
}